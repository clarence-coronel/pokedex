<template>
  <div class="flex flex-col items-center text-neutral-600 pt-10">
    ...
    <div
      ...
      class="w-full h-fit flex flex-col p-10 gap-10 md:flex-row md:flex-wrap md:justify-center md:items-center"
    >
      <template v-for="(evolution, index) in evolutionChain" :key="index">
        <EvolutionCard
          @updatePokemonData="handleClick"
          v-if="typeof evolution == 'string'"
          :name="evolution"
          :index="index"
        />
        <EvolutionCardSet
    ...
    <div
      v-else
      class="w-full flex justify-center items-center animate-pulse min-h-40"
    >
      Loading...
    </div>
  </div>
