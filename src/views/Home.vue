<template>
  <div
    class="max-w-[1400px] w-full min-h-screen flex justify-start items-center flex-col gap-5 p-5"
  >
    <div class="w-full sm:px-5 flex justify-end gap-20">
      ...
      <CustomSelect
        ...
        @valueUpdated="valueUpdatedHandler"
        :selectedPropVal="limit"
        :selectedPropKey="limit"
        :options="options"
      />
    </div>
    <div
      v-if="!isLoading"
      class="w-full min-h-[70vh] grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-10 sm:p-5"
    >
      <Card
    ...
    <div
      ...
      class="w-full min-h-[70vh] flex flex-col justify-center items-center p-5 rounded-lg bg-white"
    >
      <svg
    ...
    <div
      class="flex flex-col-reverse w-full gap-3 sm:flex-row sm:gap-5 justify-center items-center"
    >
      <button
        class="hover:bg-green-400 disabled:bg-neutral-300 duration-200 bg-green-500 rounded-md p-2 text-white font-bold w-full sm:max-w-40"
        @click="() => changePage(-1)"
...
onMounted(async () => {
  if (
    route.query.page < 1 ||
    isNaN(route.query.page) ||
    isNaN(route.query.limit) ||
    route.query.limit < 1
  ) {
    router.replace({
      path: "/",
